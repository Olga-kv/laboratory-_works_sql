...
select book.book_name as Назва, book.book_price as Ціна,publishing.publishing as Видавництво from book,publishing  where book.book_publishing_id = publishing.id_publishing
...
select b.book_name as Назва, b.book_price as Ціна,p.publishing as Видавництво, b.book_format as Формат from book b,publishing p where b.book_publishing_id = p.id_publishing
...
select c.category as категорія,sum(b.book_pages) as загальна_кількість_сторінок  from book b inner join category c on b.book_category_id=c.id_category group by c.category order by sum(b.book_pages) DESC
...
select a.book_name as Book_name_1 , a.book_pages as Amount_of_pages_1, b.book_name as Book_name_2, b.book_pages as Amount_of_pages_2 from book a ,book b	where 	a.book_pages=b.book_pages and a.book_id <> b.book_id
...
select book.book_name,category.category from book inner join category on category.id_category=book.book_category_id where book_category_id in (select id_category from category where category like '%C ++%')

--14.Вивести книги видавництва 'BHV', видані після 2000 р Використовувати підзапити (Subquery) .
select * from book  where book_publishing_id in (select id_publishing from publishing where publishing like 'BHV%') and EXTRACT (YEAR from book_date) >= '2000';
